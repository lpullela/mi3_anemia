...
# newfile = '/Users/layapullela/Projects/MI3-2022/mimic3-anemia/mimic-iii-clinical-database-1.4/CHARTEVENTS_o2.csv'
...
# print( "all pulse-ox readings written to seperate HR file! ")

import csv

warn = str( input( "WARNING!: Do you want to run this file? you will lose data in O2 avg file: " ) )
if not warn.lower() == 'yes': 
    exit()

path = '/Users/layapullela/Projects/MI3-2022/mimic3-anemia/mimic-iii-clinical-database-1.4/CHARTEVENTS_o2.csv'

#index 1 is patient_id 

newfile = '/Users/layapullela/Projects/MI3-2022/mimic3-anemia/mimic-iii-clinical-database-1.4/O2_AVG.csv'

file = open( newfile, 'w' )
HT_SIZE = 200 
o2_rates = [ [] for i in range( HT_SIZE ) ]
...
with open( path, newline='' ) as csvfile: 
    ...
    for row in reader: 
        ...
        for item in o2_rates[ pat_id % HT_SIZE ]: 
            if ( item[ 0 ] == pat_id ): 
        ...
        if create_new_entry: 
            try: 
                o2_rates[ pat_id % HT_SIZE ].append( [ pat_id, float( row[ 9 ] ), 1 ])
            except ValueError: 
                continue
